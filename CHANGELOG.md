...
- 🔄 **配置路径系统重构** - 应用数据目录从 `~/.claude/cctoolbox/` 统一迁移到 `~/.cctoolbox/`，简化路径管理逻辑
- 🔄 **配置加载简化** - 移除 `resolveConfigFile` 中 legacy 路径兼容逻辑，统一使用新路径 `~/.cctoolbox/config.json`
- 🔄 **迁移机制增强** - 新增临时目录迁移策略，迁移过程先复制到临时目录校验，再原子移动到目标目录，提升安全性

### Fixed

- 🐛 **迁移安全性和可靠性** - 迁移失败时清理临时目录而非目标目录，避免误删已存在的有效数据

### Added

- ✅ **迁移测试覆盖** - 新增 `CCTOOLBOX_HOME` 环境变量支持测试、迁移优先级测试（Claude 目录优先于 legacy 目录）
...
- 🎨 **敏感字段可见性控制** - CodeX 渠道面板新增敏感字段（API Key、Project ID）显示/隐藏切换功能，用户可自主控制敏感信息展示
- 🧰 **渠道面板抽象工厂** - 新增 `channelPanelFactories.js`，统一管理不同渠道的面板创建逻辑，提升可维护性

### Changed

- 🔄 **ChatHistoryDrawer UI 重构** - 优化消息抽屉布局和交互，支持敏感字段可见性控制状态
- 🔄 **RightPanel 面板自适应** - 右侧面板根据渠道类型动态渲染，支持敏感信息展示控制
- 🔄 **SessionSummaryCard 增强** - 会话摘要卡片增加敏感数据展示控制逻辑
- 🔄 **FilterBar 搜索控件优化** - 过滤栏组件优化，支持搜索功能集成

### Fixed

- 🐛 **渠道配置渲染修复** - 修复某些渠道配置项渲染异常的问题
...
- 🐛 **预设切换逻辑修复** - 新建渠道时切换预设正确应用名称/baseUrl/官网地址；编辑已保存渠道时保留名称但切换 baseUrl/官网地址（显示原值提示）
- 🐛 **智谱默认模型修正** - 智谱渠道默认模型改为 glm-5，仅 Haiku 模型默认为 glm-4.7
...
- 🔄 **渠道管理增强** - channels 服务增加原子写入、备份和配置检查功能； API 层统一增强支持更多渠道操作
- 🔄 **配置管理改进** - settings-manager 和 config loader 原子写入支持，提升配置操作可靠性
- 📦 **模型列表优化** - model-list 服务改进模型提取逻辑，增强国产模型支持
- ⚡ **测速功能改进** - speed-test 服务改进，支持可选模型测速

### Fixed

- 🐛 **前端渠道 API 改进** - 修复前端 API 调用参数传递问题，正确传递 channelType 参数
- 🐛 **ChannelCard 组件优化** - 简化模型加载逻辑，修复潜在的状态问题
...
- 🆕 **测速可选模型** - 新增模型列表 API（`GET /:id/models`），测速支持通过 n-select tag 模式选择或手输模型，兼容国产渠道
- 🧰 **模型列表服务** - 新增 `model-list.js` 服务模块，支持从外部 API 获取渠道可用模型列表及缓存机制

### Changed

- 🔄 **拖拽排序体验优化** - 重构拖拽相关 CSS 样式，优化 ghost/chosen/drag-fallback 视觉反馈，禁用拖拽期间文字选中
- 🔄 **拖拽自动滚动** - 为 `<draggable>` 添加 `scroll-sensitivity` / `scroll-speed` 配置，列表过长时可自动滚动容器

### Fixed

- 🐛 **空模型字段残留** - 切换到模型字段为空的渠道时，`settings.json` 不再保留上一个渠道的模型值
- 🐛 **拖拽不自动滚动** - 修复渠道列表过多时拖拽排序无法滚动容器的问题
