...
function getBaseDir() {
  ...
  const legacyStatsFile = path.join(appDir, 'gemini-statistics.json');
  const legacyDailyDir = path.join(appDir, 'gemini-daily-stats');

  const targetStatsFile = path.join(statsDir, 'gemini-statistics.json');
  const targetDailyDir = path.join(statsDir, 'gemini-daily-stats');
