...
const { parseRealProjectPath, hasActualMessages } = require('../services/sessions');
...
router.post('/generate-alias', async (req, res) => {
  ...
  try {
    ...
    if (!hasActualMessages(sessionFile)) {
      return res.status(404).json({ error: '会话没有可用于生成别名的对话内容' });
    }
    ...
    const contextTokens = resolveContextTokens(provider || config.defaultProvider);
    ...
    const tags = Array.isArray(parsed.tags)
      ? parsed.tags.map((tag) => normalizeText(tag)).filter(Boolean).slice(0, 6)
      : [];
    ...
    if (error?.message === 'Invalid project path' || error?.message?.includes('Invalid')) {
