...
| 功能                  | 描述                                                                                        |
| --------------------- | ------------------------------------------------------------------------------------------- |
| **智能会话管理**      | 自动识别 Claude/Codex/Gemini 历史会话，支持命名、搜索、Fork 分支、标签分类，内存缓存加速加载 |
| **对话详情增强**      | 工具调用可视化渲染（含 Codex 工具支持）、消息类型过滤、后端颜色区分、智能折叠和虚拟滚动优化、Subagent 详情查看 |
| **会话回收站**        | 7天误删保护，支持批量删除（实时进度反馈）、恢复和永久清理                                   |
